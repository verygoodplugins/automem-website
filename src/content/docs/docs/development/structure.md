...
graph TB
    ...
    CLI --> COPILOT["copilot.ts"]
    CLI --> HERMES["hermes.ts"]
    CLI --> MIGRATE["migrate.ts"]
    CLI --> UNINSTALL["uninstall.ts"]
    CLI --> CLOUD["cloud/\n(hosted provisioning)"]
...
├── copilot/                 # GitHub Copilot instructions, hooks, scripts, profiles
├── hermes/                  # Hermes rules + memory provider plugin (provider/)
├── antigravity/
│   └── mcp_config.json      # Antigravity MCP server config
├── cursor/
│   ├── automem.mdc.template # Cursor rule template
│   ├── user-rules.md        # Cursor user-rules snippet
│   └── rule-evals.md        # Rule evaluation notes
├── codex/
│   ├── memory-rules.md      # Codex agent rules
│   └── config.toml          # Server config for ~/.codex/
└── openclaw/
    ├── OPENCLAW_SETUP.md    # OpenClaw setup guide
    ├── skill/               # Current OpenClaw skill payload
    ├── skill-mcp/           # MCP-mode skill payload
    └── skill-legacy/        # Legacy skill payload
```
...
| Rule Files | `.md`, `.mdc` | AI assistant instructions | Cursor, Codex, OpenClaw, Copilot, Hermes |
