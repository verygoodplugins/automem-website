...
AutoMem is a Flask-based HTTP API service that provides persistent, queryable memory storage for AI assistants. Unlike traditional RAG systems that retrieve similar documents, AutoMem builds knowledge graphs with typed relationships, enabling multi-hop reasoning, temporal awareness, and pattern learning. Its internal harness records **87.00% on LongMemEval full** with **97.00% recall@5**, and **84.74% on LoCoMo full**; these are internal engineering baselines, not cross-system comparisons. For those comparisons, use the neutral Agent Memory Benchmark. See [Benchmarks](/benchmarks/) for the evidence and caveats.
...
| **Health Monitor** | `scripts/health_monitor.py` | Drift detection, webhook alerts | `HealthMonitor`, `check_consistency`, `trigger_recovery` |
...
- **SyncWorker**: Monitors drift between FalkorDB and Qdrant and queues every memory missing from Qdrant for re-embedding — there is no percentage threshold (the 5% figure is `HEALTH_MONITOR_DRIFT_THRESHOLD`, which gates alerting in `scripts/health_monitor.py`)
