...
Detection looks for `~/.codex`, `~/.claude`, `~/.cursor`, `~/.openclaw`, `~/.hermes` (or `$HERMES_HOME`), and `~/.grok` (or `$GROK_HOME`). Anything not detected is still installable — just check it.
...
Without a TTY and without `--yes`, the installer prints the plan and stops, so an unattended pipe can never make unreviewed changes — pass `--yes` (or `AUTOMEM_YES=1`) when you actually want a headless run to apply. Setting `CI`, `CODEX`, or `CLAUDE_CODE` only suppresses the splash animation; it does not imply `--yes`.
:::
...
On Railway, `memory-service` **must** set `PORT=8001`. AutoMem binds whatever `PORT` holds (falling back to `8001` only when it is unset), so a platform-injected `PORT` moves the API off the port the other services dial and you'll see `ECONNREFUSED` errors. The [Railway template](/docs/deployment/railway/) sets this for you.
:::
...
| `ECONNREFUSED` on Railway | `PORT` unset or injected with a different value | Set `PORT=8001` on `memory-service` |
