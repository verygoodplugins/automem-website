...
:::tip
If you omit `type` when storing a memory, `MemoryClassifier` classifies it automatically: regex patterns are tried first, and an LLM call is used only as a fallback when no pattern matches. If the LLM is unavailable too, the memory is stored as `Memory` with confidence `0.3`.
:::
...
4. FalkorDB stores the canonical memory record via a MERGE query
5. Background enrichment is queued via `ServiceState.enrichment_queue`
6. Embedding generation is queued to the background embedding worker, which later upserts the vector into Qdrant (the response reports `embedding_status: "queued"`). If the caller supplied an `embedding`, it is upserted into Qdrant inline instead (`embedding_status: "provided"`); if Qdrant is not configured, the step is skipped
